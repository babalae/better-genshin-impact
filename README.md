...
## 看门狗（BetterGI.Watchdog）高级配置

当你使用「一条龙」功能时，可以同时运行一个看门狗进程，用来监控 BetterGI 与原神进程是否崩溃并自动重启。

自 vX.X.X 起，看门狗额外支持：

1. **记录 BetterGI / 原神 进程的内存占用日志（可开关）**
2. **当任一进程内存占用超过阈值时，自动 Kill + Restart（一条龙恢复）**

### 配置文件位置

- 默认路径：`BetterGI\User\watchdog.config.json`
- 文本格式，UTF-8 编码，**无需界面**，手动创建/编辑该文件即可。

### 配置项说明

```jsonc
{
  // 是否启用资源占用日志（主要是内存），默认 false
  "enableLogging": true,

  // 内存阈值（单位：MB）。
  // 当 BetterGI 或 原神 任一进程的工作集内存 > 该值时，
  // 看门狗会认为发生高内存异常：
  // 1. 记录一条高内存日志；
  // 2. 杀死 原神 + BetterGI 进程；
  // 3. 如果 config.json 中仍有一条龙目标配置，则按一条龙参数重启 BetterGI。
  // 小于等于 0 表示不启用该逻辑。
  "memoryThresholdMB": 4096,

	  // 日志文件名或路径（基础名）：
	  // - 如果是相对路径（默认 "watchdog.log"），会先映射到 BetterGI\User 目录下；
	  // - 如果是绝对路径，则直接使用该路径作为基础路径；
	  // 程序每次启动时，会在基础文件名后自动追加一次性的时间戳后缀 -yyyyMMdd-HHmmss，
	  // 例如：watchdog-20250101-120000.log，这样每次启动会生成一个新的日志文件。
...
> [!TIP]
> - 如果你只想记录日志但不杀进程：
>   - `enableLogging` 设为 `true`，`memoryThresholdMB` 设为 `0` 或删除该字段即可。
> - 如果你只想在高内存时自动重启：
>   - `enableLogging` 可设为 `false`，只保留 `memoryThresholdMB` 即可（高内存触发时仍会在控制台输出一条提示）。
