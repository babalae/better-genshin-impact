...
2. 默认截图模式下，HDR、Windows Auto HDR 和 N 卡滤镜等画面处理会影响识别。检测到 HDR 环境时，建议切换到 `WindowsGraphicsCapture（HDR）` 或手动关闭 HDR / Auto HDR；若仍继续使用当前模式，可能出现颜色异常、识别失败或任务不稳定。游戏亮度请保持默认。
