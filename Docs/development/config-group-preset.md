...
预设配置组是随 BetterGI 本体发布的配置组模板，用于快速创建一组已经编排好的调度器任务。预设配置组属于本体资源，不放入 Javascript 脚本仓库，也不会参与脚本仓库订阅和更新。

## 目录结构

在本体项目中，每个预设配置组使用一个以预设配置组名称命名的目录：
...
目录名、`manifest.json` 中的 `name`、配置组 JSON 中的 `name` 必须完全一致。预设配置组目录下至少应包含以下三个文件：

- `manifest.json`：声明预设配置组的基本信息和依赖。
- 配置组 JSON：用于导入调度器，通常命名为 `<预设配置组名称>.json`。
- `README.md`：在预设配置组浏览窗口中展示使用说明、注意事项和配置建议。
...
| `id` | 是 | 预设配置组的稳定标识，建议使用 `preset.<作者或组织>.<名称>`，发布后不要随意修改。 |
| `name` | 是 | 预设配置组显示名称，同时必须与目录名和配置组 JSON 的 `name` 一致。 |
| `version` | 是 | 预设配置组版本号，建议使用语义化版本。 |
| `min_bgi_version` | 否 | 支持的最低 BetterGI 版本说明。目前用于声明信息，不会自动阻止应用。 |
| `description` | 否 | 列表和详情页显示的简介。 |
| `authors` | 否 | 作者名称列表。 |
| `config_group_file` | 否 | 配置组 JSON 的相对路径，默认为 `<name>.json`。路径必须位于当前预设配置组目录内。 |
...
- `min_version` 仅用于 Javascript 依赖，表示允许应用预设配置组的最低脚本版本。留空时只检查脚本是否安装，以兼容已有预设配置组。

应用预设配置组时只进行本地完整性检查。Javascript 依赖填写 `min_version` 后，会读取脚本 `manifest.json` 中的 `version` 并按语义化版本比较；脚本版本、最低版本无法解析或实际版本较低时，均视为依赖不满足。界面会列出缺失或版本不满足的依赖并提示用户前往订阅，不会自动订阅、跳转或修改订阅状态。
...
README 中的图片和其他 Markdown 相对链接应放在当前预设配置组目录内，并使用相对路径引用。例如：

```markdown
![流程示意图](images/flow.png)
```

详情窗口会以预设配置组目录作为 Markdown 基路径解析这些链接。
...
7. 安装依赖后应用，调度器中会新增同名配置组；再次应用同一预设配置组会提示配置组已存在，不会覆盖用户配置。

完成以上检查后，再将整个预设配置组目录随本体资源提交。不要将其复制到脚本仓库或订阅目录中。
