...
**⚠️注意：**
1. 切換遊戲解析度/Windows HDR，或調整「SDR 內容亮度」後，請重啟本軟件；將遊戲視窗移至另一台顯示器時，WGC HDR 會自動刷新。
2. 不支援 N 卡濾鏡等會改變畫面顏色的濾鏡。HDR 遊戲畫面需選擇 `WindowsGraphicsCapture（HDR）`，程式會將 FP16 HDR 輸入正規化為辨識所需的 SDR。
