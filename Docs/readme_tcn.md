...
**⚠️注意：**
1. 視窗大小變化、切換遊戲解析度/顯示器/Windows HDR，或調整「SDR 內容亮度」後，請重啟本軟件。
2. 不支援 N 卡濾鏡等會改變畫面顏色的濾鏡。HDR 遊戲畫面需選擇 `WindowsGraphicsCapture（HDR）`，程式會將 FP16 HDR 輸入正規化為辨識所需的 SDR；`BitBlt` 不提供原生 HDR。
