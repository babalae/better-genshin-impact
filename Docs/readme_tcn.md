...
2. 預設截圖模式下，HDR、Windows Auto HDR 與 N 卡濾鏡等畫面處理會影響辨識。偵測到 HDR 環境時，建議切換到 `WindowsGraphicsCapture（HDR）` 或手動關閉 HDR / Auto HDR；若仍繼續使用目前模式，可能出現顏色異常、辨識失敗或任務不穩定。遊戲亮度請保持預設。
