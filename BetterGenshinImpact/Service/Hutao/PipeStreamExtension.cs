...
internal static class PipeStreamExtension
{
    ...
    public static void WritePacketWithJsonContent<TData>(this PipeStream stream, byte version, PipePacketType type, PipePacketCommand command, TData data)
    {
        ...
        // 使用运行时类型序列化，避免 TData 被推断为基类 PipeResponse 时擦除 PipeResponse<T> 的 Data 字段。
    ...
    public static void WriteResponse<T>(this PipeStream stream, PipeResponse<T> response)
    {
        stream.WritePacketWithJsonContent(HutaoPipeProtocol.Version, PipePacketType.Response, PipePacketCommand.BetterGenshinImpactToSnapHutaoResponse, response);
        stream.Flush();
    }
}
