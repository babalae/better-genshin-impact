...
public static class OcrUtils
{
...

    {
        ...
        return new DenseTensor<float>(
            memory: tensorMemoryOwner.Memory[..(int)total],
            dimensions: new[] { 1, channels, data.Rows, data.Cols }
        );
    }
    ...
    {
        using var rt = new ResourcesTracker();
        int imgC = (int)image_shape.Channel;
        int imgH = (int)image_shape.Height;
        int imgW = (int)image_shape.Width;
    ...
    resized_image /= 0.5
        ...
        return new DenseTensor<float>(
            memory: tensorMemoryOwner.Memory[..(int)total],
            dimensions: new[] { 1, resizedImage.Channels(), resizedImage.Rows, resizedImage.Cols }
        );
    }
