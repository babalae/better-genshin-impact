...
/// 加载全部地区模板，建 areaName → RecognitionObject 映射。
/// 生产路径只对目标地区执行主模板和 b 变体模板匹配，搜索区域为国家列表所在的右侧 1/3。
/// 模板不绑定固定格子，直接在整块 ROI 内寻找最佳命中，适配地区顺序变化。
///
...
internal sealed class SwitchAreaRegionAssets
{
    ...
    private static RecognitionObject? TryBuildFile(string areaName, string templateFile, int w, int h, Rect roi)
    {
        ...
        try
        {
            mat = GameTaskManager.LoadAssetImage("QuickTeleport", templateFile, w, h);
            ...
            // Mat 的所有权转移给 RecognitionObject，由缓存持有直到进程结束。
            mat = null;
