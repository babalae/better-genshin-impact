...
public class AutoFightTask : ISoloTask
{
    ...
    /// </summary>
    /// <returns></returns>
    private async Task<Boolean> initChatButton(ImageRegion ra)
    {
        ...
        if (enterIconMat != null) { 
            ...
            Logger.LogInformation("enterIcon 初始化成功");
            return true;
        }
        Logger.LogInformation("enterIcon 初始化失败");
        return false;
    }
    ...
    /// <returns></returns>
    private async Task<Mat> findEnterIcon(Mat scrMap,int x,int y, Vec3b basePixel,int tolerance,int maxSearchWidth,int Height = 0)
    {
    ...
    private bool IsPixelMatch(Vec3b pixel1, Vec3b pixel2, int tolerance)
    {
