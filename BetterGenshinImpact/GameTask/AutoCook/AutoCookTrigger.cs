...
public class AutoCookTrigger : ITaskTrigger
{
    ...
    public string Name => Lang.S["Trigger_062_07ba68"];
        ...
        content.CaptureRectArea.Find(ElementAssets.Instance.UiLeftTopCookIcon, _ =>
        {
            ...
            if (perfectBarRects.Count >= 2)
            {
                ...
                if (btnList.Count >= 1)
                {
                    if (btnList.Count > 1)
                    {
                        _logger.LogWarning(Lang.S["GameTask_10381_22318b"], "识别到多个结束烹饪按钮");
                    ...
                    region.ClickTo(x, y);
                    _logger.LogInformation(Lang.S["GameTask_10381_22318b"], "点击结束按钮");
                }
            }
        });
    }
