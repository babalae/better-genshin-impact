...
public static class FarmingStatsRecorder
{
    ...
    public static bool IsDailyFarmingLimitReached(FarmingSession farmingSession, out string message)
    {
        ...
        if (isEliteOverLimit) messages.Add($"{Lang.S["GameTask_11720_4bc21c"]});
        if (isNormalOverLimit) messages.Add($"{Lang.S["GameTask_11719_0ba244"]});
        
        //尝试更新米游社的条件，超过最后札记两个小时，并且超过上次尝试更新20分钟
        debugInfo($"{Lang.S["GameTask_11718_aa8723"]});
        if (mysEnable
        ...
        {
            messages.Add(Lang.S["GameTask_11717_954919"]);
            message = string.Join(",", messages);
            return true;  
        }
        ...
        {
            messages.Add(Lang.S["GameTask_11716_2707c6"]);
            message = string.Join(",", messages);
            return true;  
        }
        ...
        {
            result = true;
            if (farmingSession.NormalMobCount > 0) messages.Add(Lang.S["GameTask_11715_37ff46"]);
        }
        else if (farmingSession.PrimaryTarget == "normal" && isNormalOverLimit)
        {
            result = true;
            if (farmingSession.EliteMobCount > 0) messages.Add(Lang.S["GameTask_11714_277e57"]);
        }
    ...
    public static void RecordFarmingSession(FarmingSession session, FarmingRouteInfo route)
    {
        try
        {
            ...
            TaskControl.Logger.LogInformation(
                $"{Lang.S["GameTask_11713_26bd42"]} +
                $"{Lang.S["GameTask_11711_404c96"]}+(dailyData.EnableMiyousheStats()?"(合并米游社数据)":""));
        }
        catch (Exception e)
        {
            TaskControl.Logger.LogError($"{Lang.S["GameTask_11710_31aa10"]});
        }
    }
    ...
    public async static Task TryUpdateTravelsData()
    {
        ...
        try
        {
           ...
           // await _updateLock.WaitAsync(); // 获取独占锁    
            debugInfo(Lang.S["GameTask_11709_5163ac"]);
                ...
                && cookie != string.Empty)
            {
                try
                {
                    ...
                    if (actionItems.Count > 0)
                    {
                        ...
                        debugInfo($"{Lang.S["GameTask_11708_5bfa62"]});
                    }
                    else
                    {
                        TaskControl.Logger.LogError(Lang.S["GameTask_11707_7ad442"]);
                    }
                }
                catch (Exception e)
                {
                    TaskControl.Logger.LogError($"{Lang.S["GameTask_11706_df307f"]});
                }
            }
