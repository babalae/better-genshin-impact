...
public partial class MacroSettingsPageViewModel : ViewModel
{
    ...
    [ObservableProperty]
    private string _keyboardMonitorHint = Tr("当前为虚拟键盘输出，监听面板不会读取硬体状态。");

    [ObservableProperty]
    private string _mouseMonitorHint = Tr("当前为虚拟滑鼠输出，监听面板不会读取硬体状态。");
    ...
    public MacroSettingsPageViewModel(IConfigService configService, INavigationService navigationService)
    {
        ...
        InputModeOptions =
        [
            new(HardwareInputConfigValues.Virtual, Tr("虚拟输入")),
            new(HardwareInputConfigValues.Hardware, Tr("硬体")),
        ];
        KeyboardHardwareVendorOptions =
        [
            new(HardwareInputConfigValues.Ferrum, "Ferrum"),
            new(HardwareInputConfigValues.Makxd, "Makxd"),
        ];
        MouseHardwareVendorOptions =
        [
        ...
        [
            CreateMouseMonitor(Tr("左键"), HardwareMouseButton.Left),
            CreateMouseMonitor(Tr("右键"), HardwareMouseButton.Right),
            CreateMouseMonitor(Tr("中键"), HardwareMouseButton.Middle),
            CreateMouseMonitor(Tr("侧键1"), HardwareMouseButton.Side1),
            CreateMouseMonitor(Tr("侧键2"), HardwareMouseButton.Side2),
        ];
    ...
    private void RefreshKeyboardMonitor()
    {
        ...
        if (!hardwareConfig.IsKeyboardHardware)
        {
            ...
            KeyboardMonitorHint = Tr("当前为虚拟键盘输出，监听面板不会读取硬体状态。");
        ...
        {
            IsKeyboardMonitorSupported = false;
            KeyboardMonitorHint = Tr("未连接到键盘硬体，暂时无法读取按键状态。");
            ResetMonitorItems(KeyboardMonitorItems);
            return;
        }
        ...
        {
            IsKeyboardMonitorSupported = false;
            KeyboardMonitorHint = Tr("当前键盘 API 暂不支援状态监听。");
            ResetMonitorItems(KeyboardMonitorItems);
            return;
        }

        IsKeyboardMonitorSupported = true;
        KeyboardMonitorHint = Tr("状态映射：0=未触发 1=物理 2=硬体 3=同时");
    ...
    private void RefreshMouseMonitor()
    {
        ...
        if (!hardwareConfig.IsMouseHardware)
        {
            ...
            MouseMonitorHint = Tr("当前为虚拟滑鼠输出，监听面板不会读取硬体状态。");
        ...
        {
            IsMouseMonitorSupported = false;
            MouseMonitorHint = Tr("未连接到滑鼠硬体，暂时无法读取按键状态。");
            ResetMonitorItems(MouseMonitorItems);
            return;
        }
        ...
        {
            IsMouseMonitorSupported = false;
            MouseMonitorHint = Tr("当前滑鼠 API 暂不支援状态监听。");
            ResetMonitorItems(MouseMonitorItems);
            return;
        }

        IsMouseMonitorSupported = true;
        MouseMonitorHint = Tr("状态映射：0=未触发 1=物理 2=硬体 3=同时");
