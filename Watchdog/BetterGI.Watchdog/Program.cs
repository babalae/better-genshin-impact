    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var baseLogPath = Path.IsPathRooted(logFileName)
                ? logFileName
                : Path.Combine(betterGiDir, "log", logFileName);
            ...
            {
                // 计算实际生效的阈值
                long effectiveGenshinThreshold = watchdogConfig.GenshinMemoryThresholdMB > 0
                    ? watchdogConfig.GenshinMemoryThresholdMB
                    : watchdogConfig.MemoryThresholdMB;
                long effectiveBetterGIThreshold = watchdogConfig.BetterGIMemoryThresholdMB > 0
                    ? watchdogConfig.BetterGIMemoryThresholdMB
                    : watchdogConfig.MemoryThresholdMB;

                Console.WriteLine(
                    $"[Watchdog] Resource logging enabled. LogFile='{watchdogLogPath}', " +
                    $"GenshinMemoryThresholdMB={effectiveGenshinThreshold}, " +
                    $"BetterGIMemoryThresholdMB={effectiveBetterGIThreshold}");
            }
            ...
            string oneDragonConfigPath)
        {
            ...
            // 分别获取两个进程的内存阈值，如果未单独设置则使用统一的 MemoryThresholdMB
            long genshinThresholdMB = config.GenshinMemoryThresholdMB > 0
                ? config.GenshinMemoryThresholdMB
                : config.MemoryThresholdMB;
            long betterGiThresholdMB = config.BetterGIMemoryThresholdMB > 0
                ? config.BetterGIMemoryThresholdMB
                : config.MemoryThresholdMB;

            long genshinThresholdBytes = genshinThresholdMB > 0 ? genshinThresholdMB * 1024L * 1024L : 0L;
            long betterGiThresholdBytes = betterGiThresholdMB > 0 ? betterGiThresholdMB * 1024L * 1024L : 0L;

            // 如果既不开日志，也没有配置阈值，就直接跳过
            if (!needLog && genshinThresholdBytes <= 0 && betterGiThresholdBytes <= 0)
            {
                return false;
            }
            ...
            bool overGenshin = genshinThresholdBytes > 0 && genshinWs > genshinThresholdBytes;
            bool overBetterGi = betterGiThresholdBytes > 0 && betterGiWs > betterGiThresholdBytes;

            if (overGenshin || overBetterGi)
            {
                string reason =
                    $"Detected high memory usage. " +
                    $"GenshinThreshold={genshinThresholdMB}MB, BetterGIThreshold={betterGiThresholdMB}MB, " +
                    $"YuanShen_WS={BytesToMb(genshinWs)}MB, " +
                    $"BetterGI_WS={BytesToMb(betterGiWs)}MB, " +
        ...
        private static long ParseLongValue(JsonElement value, long defaultValue)
        {
            if (value.ValueKind == JsonValueKind.Number)
            {
                ...
                if (value.TryGetDouble(out var d))
                {
                    return (long)d;
                     ...
                     long.TryParse(value.GetString(), out var l2))
            {
                return l2;
            }
            return defaultValue;
        ...
        private static WatchdogConfig LoadWatchdogConfig(string path)
        {
            try
            {
                ...
                using var doc = JsonDocument.Parse(jsonBytes, docOptions);
                var root = doc.RootElement;
                ...
                foreach (var prop in root.EnumerateObject())
                {
                    ...
                    switch (name)
                    {
                        case "enablelogging":
                            ...
                            {
                                cfg.EnableLogging = prop.Value.GetBoolean();
                            }
                            else if (prop.Value.ValueKind == JsonValueKind.String &&
                                    bool.TryParse(prop.Value.GetString(), out var b))
                            {
                                cfg.EnableLogging = b;
                            }
                            break;
                        case "memorythresholdmb":
                            cfg.MemoryThresholdMB = ParseLongValue(prop.Value, cfg.MemoryThresholdMB);
                            break;
                        case "genshinmemorythresholdmb":
                            cfg.GenshinMemoryThresholdMB = ParseLongValue(prop.Value, cfg.GenshinMemoryThresholdMB);
                            break;
                        case "bettergimemorythresholdmb":
                            cfg.BetterGIMemoryThresholdMB = ParseLongValue(prop.Value, cfg.BetterGIMemoryThresholdMB);
                            break;
                        case "logfilename":
                            if (prop.Value.ValueKind == JsonValueKind.String)
                            {
                                cfg.LogFileName = prop.Value.GetString();
                            }
                            break;
                    }
                }
...
internal class WatchdogConfig
{
    ...
    /// 小于等于 0 表示不启用该功能。
    /// 注意：此属性作为向后兼容的默认值。如果设置了 GenshinMemoryThresholdMB 或 BetterGIMemoryThresholdMB，则优先使用各自的阈值。
    /// </summary>
    public long MemoryThresholdMB { get; set; } = 0;

    /// <summary>
    /// 原神进程的内存占用阈值（MB）。当原神 WorkingSet 超过该值时，
    /// 将视为异常状态，触发一次 Kill+Restart。
    /// 小于等于 0 表示使用 MemoryThresholdMB 的值作为默认值。
    /// </summary>
    public long GenshinMemoryThresholdMB { get; set; } = 0;

    /// <summary>
    /// BetterGI 进程的内存占用阈值（MB）。当 BetterGI WorkingSet 超过该值时，
    /// 将视为异常状态，触发一次 Kill+Restart。
    /// 小于等于 0 表示使用 MemoryThresholdMB 的值作为默认值。
    /// </summary>
